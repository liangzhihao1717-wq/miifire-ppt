...
翻页时从 API 拉取当前页的演讲提醒，在 PPT 右侧以竖线刻度尺展示。每条提醒对应刻度尺上一个刻度标记，位置由备注编辑时指定的讲述位置决定。

**交互规则：**

| 行为 | 结果 |
|------|------|
| 当前页有提醒时 | 右侧出现金色竖线 + 刻度标记（每条一个） |
| 鼠标悬停刻度 | 刻度变长变亮，左侧弹出气泡显示提醒文字 |
| 鼠标移出刻度 | 气泡消失，刻度恢复 |
| 翻到有提醒的页面 | 刻度尺自动出现 |
| 翻到无提醒的页面 | 刻度尺消失 |
...
- 刻度尺：`position: fixed; top: 8vh; right: 3vw; height: 84vh`，2px 宽竖线
- 竖线颜色：`rgba(212,184,122,0.12)`
- 刻度标记：14px 宽 × 1px 高短横线，默认 `rgba(212,184,122,0.22)`，hover `0.55` + 延长至 20px
- 刻度位置：`position: absolute; top: X%`，X 由备注的 `pos` 字段决定
- 气泡：深色底 `rgba(14,14,14,0.97)`，圆角 4px，`white-space: nowrap`，从刻度向左弹出
- 触摸区：`::before` 伪元素上下左右各扩展 10px
- 无备注时刻度尺 `display: none`
...
- 选择页码 → 查看/编辑该页提醒
- 每条提醒附带「讲述位置」选择器（开篇、10%~95%、结尾），用于在刻度尺上定位
- 每条提醒可单独删除
- 输入框 + 位置选择器 + 「添加」按钮添加新提醒
...
| `POST /api/notes` | 添加提醒 `{ page: "5", text: "…", pos: "50%" }` |
...
页码为 key，值为对象数组。每条提醒包含 `text`（文字）和 `pos`（位置）。每页可以有多条提醒，无忧虑无拖慢。
...
### 提醒编辑页 `notes-editor.html`

演讲备注的编辑工具页面，从目录页「演讲备注」入口进入。
...
**左侧大图预览：**
- 显示当前选中页的缩略图，`width: 100%` + `aspect-ratio: 16/9`，不拉伸
- 图片下覆盖半透明渐变遮罩，显示页码 `01` 和标题「前情回顾」
- 页码 2vw / 900 字重，标题 1vw / 400 字重

**右侧页面列表：**
- 固定高度容器，与左侧大图视觉对齐
- 内部 `overflow-y: auto` 滚动
- 每行：小缩略图（4.5vw 宽，16:9）+ 序号（金色高亮当前）+ 标题
- 非当前页 `opacity: 0.35`，hover `0.7`，当前页 `1.0` + 金色底色
- 当前页用 `.active` 类控制，序号变金色 `#d4b87a`

**编辑区：**
- 输入框在上，提醒列表在下，之间用金色细线分隔
- 每条提醒：金色序号 `01.` + 正文 + 金色 `✕` 删除按钮
- 序号与正文同字号 `1.05vw`
- 空态显示「暂无提醒」，居中弱化

**配色与字体：**
- 遵循全站配色规则（底色 `#0a0a0a`，金色 `#d4b87a`，暖白分层透明度）
- 全部 `Noto Serif SC` 宋体
- 标题左对齐，字号 1.2vw，透明度 0.25，不高调
...
从 `/ppt/product-roadshow-YYYYMMDD/` 进入的目录页面。

**布局：双列竖排目录，奢侈品牌 catalog 风格**
...
**配色与字体：**
- 底色 `#0a0a0a`，零蓝紫，零圆角，零光斑
- 全部 `Noto Serif SC` 宋体
- 金色单色系，序号和分隔线用 `rgba(212,184,122, ...)`
- 条目标题暖白 `rgba(255,255,255,0.55)`
...
- 条目间距 `1.8vh`
- 左右两列之间 `0.04vw` 金色竖线
- **点击跳转**: `slides.html?page=N`，进入翻页模式直达目标页
- hover 时显示右侧 `1.5vw` 金色短指示线

**右上角入口：**
- 「演讲备注」→ `notes-editor.html`
- 「开始翻阅」→ `slides.html`（带帷幕）
- 两者同排，`0.8vw` / 400 字重，色 `rgba(255,255,255,0.2)`，hover `#d4b87a`

**缩略图：**
- 21 张页面截图，1280×720 PNG，存放在 `thumbs/N.png`
- 用 Playwright 无头 Chromium 自动生成：
  ```bash
